...
class Main extends CI_Controller {
	...
	public function index()
	{
        ...
        // Message details
        $numbers = array(7507383064,);
        $sender = urlencode('GGEEKS');
	...
	public function shippingcharge(){
        $data = $this->session->userdata;
        $data['result'] = $this->user_model->getAllSettings();
    ...
    public function updateCharges(){
        ...
        $response = $this->user_model->updateSettingCharge($_POST['charges']);
        if($response){
            $result = 'Charges Updated Succesfully';
        }
    ...
    public function adduser()
    {
	    ...
	    if($dataLevel == "is_admin"){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($this->user_model->isDuplicateContact($this->input->post('mobile'))){
                    $this->session->set_flashdata('flash_message','User Conatct already exists');
                    redirect(site_url('main/adduser'));
                }
                else{
                    $this->load->library('password');
    ...
    public function myorders()
    {
        ...
        if(empty($this->session->userdata['email'])){
            ...
            $data['myorders'] = $this->user_model->getLoginUserOrder($data['id']);
            $this->load->view('header', $data);
            $this->load->view('navbar', $data);
            $this->load->view('container');
